...
bool check_reinjection_multiple(
    ...
    for (unsigned int i = 0; i < npoints; i++){
        for (unsigned int j = 0; j < size; j++){
            if (xn[i] < fixed_points[i][j] - c || xn[i] > fixed_points[i][j] + c){
                if (xn1[i] > fixed_points[i][j] - c && xn1[i] < fixed_points[i][j] + c){
                    count++;
                    break;
                }
            }
...
bool check_reinjection_circular_multiple(
    long double **fixed_points,
    long double *xn,
    long double *xn1,
    unsigned int size,
...
bool check_ejection_circular_multiple(
    long double **fixed_points,
    long double *xn,
    long double *xn1,
    unsigned int size,
...
void reinject_states_fixedpoints_multipoints(
    ...
    for (long unsigned int i = 0; i < nevol; i++){
        ...
        if (check_reinjection_circular_multiple(fixed_points, xn, xn1, 2, npoints, c) == 1){
            xr[rcount] = xn1[0];
            yr[rcount] = xn1[1];
            rcount++;
        }
...
int main() {
    ...
    fp1 = fopen("../datafiles/test_kimkye_reinjected_prev_fixed_points_multiple.dat", "w");
