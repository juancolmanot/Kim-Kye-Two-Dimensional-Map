...
int main() {
    ...
    fp1 = fopen("../datafiles/test_kimkye_rotate.dat", "w");
    fp2 = fopen("../datafiles/test_kimkye_m_err.dat", "w");
    
    long unsigned int N = 500000000000;
    unsigned int ntarget = 1000;
    ...
    for (unsigned int i = 0; i < finalsize; i++){
        fprintf(fp1, "%5.10Lf %5.10Lf %5.10Lf %5.10Lf\n", xr[i], yr[i], xr_rot[i], yr_rot[i]);
    }
    
    for (unsigned int i = 0; i < errsize; i++){
        fprintf(fp2, "%d %5.10Lf\n", i, errors[i]);
    }
