...
int main(int argc, char *argv[]) {
    ...
    // ===============================================================================
    Parameters4 params4;
    load_parameters_from_file(params_file, &params4, handler4);
    ...
    long double al0 = params4.alpha0, aln = params4.alphan;
    unsigned int nal = params4.nalphas;
    long double dalpha = (aln - al0) / (long double)(nal - 1);

    for (unsigned int i = 0; i < params4.nalphas; i++) {

        // ===============================================================================
        // Instanciate parameters for system.
        // ===============================================================================
        Parameters_kimkye *params = malloc(sizeof(Parameters_kimkye));
        params->alpha = params4.alpha0 + (long double)i * dalpha;
        params->beta = params4.p_beta;
        ...
        // ===============================================================================
        for (unsigned int j = 0; j < (params4.transient + params4.nevol); j++){
            map_n(map_kimkye, xn1, xn, (int)params4.n_map, params);
            rotate_state(xn, xnrot, theta);
            if (j > params4.transient) {
                fprintf(f, "%Lf %Lf %Lf\n", params->alpha, xnrot[0], xnrot[1]);    
            }
            xn[0] = xn1[0];
            xn[1] = xn1[1];
        }
