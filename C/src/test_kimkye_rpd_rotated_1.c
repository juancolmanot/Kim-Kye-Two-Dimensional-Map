...
void rpd_funct(
    ...
    for (long unsigned int i = 0; i < nevol; i++){
        ...
        if (d < threshold && laminar == 0){// && xn1[variable] < upperbound && xn1[variable] > lowerbound) {
            ...
            rcount++;
            if (rcount % 1000 == 0) {
                printf("reinjections: %5.10Lf %%\n", (long double)rcount * 100 / (long double)ntarget);
            }
        }
...
int main() {
    ...
    fp1 = fopen("../datafiles/test_kimkye_rotate_rpd_3d_region5.dat", "w");
    fp2 = fopen("../datafiles/test_kimkye_rpd_3d_region5.dat", "w");
    
    long unsigned int N = 500000000000;
    unsigned int ntarget = 20000;
    ...
    long double *xr, *yr, *xr_rot, *yr_rot;

    xr = calloc((size_t)ntarget, sizeof(long double));
    yr = calloc((size_t)ntarget, sizeof(long double));
    
    xr_rot = calloc((size_t)ntarget, sizeof(long double));
    yr_rot = calloc((size_t)ntarget, sizeof(long double));
    ...
    long double *xbins, *ybins;
    long double **zbins = (long double**)calloc(nbins, sizeof(long double));
    for (unsigned int i = 0; i < nbins; i++){
        zbins[i] = (long double*)calloc(nbins, sizeof(long double));
    }
    xbins = calloc(nbins, sizeof(long double));
    ybins = calloc(nbins, sizeof(long double));
    ...
    for (unsigned int i = 0; i < nbins - 1; i++){
        for (unsigned int j = 0; j < nbins - 1; j++){
            fprintf(fp2, "%3.7Lf %3.7Lf %3.7Lf\n", xbins[i], ybins[j], zbins[i][j]);
        }
    }

    long double theta0 = 10;
    long double tol = 0.005;
    unsigned int max_iter = 100;
    
    long double *errors = calloc(max_iter, sizeof(long double));
    unsigned int errsize = max_iter;
    ...
    zbins = (long double**)calloc(nbins, sizeof(long double));
    for (unsigned int i = 0; i < nbins; i++){
        zbins[i] = (long double*)calloc(nbins, sizeof(long double));
    }
    xbins = calloc(nbins, sizeof(long double));
