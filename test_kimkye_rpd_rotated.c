...
void rotate_state(
    ...
    long double thetarad;
    thetarad = theta * 2 * M_PI / 360;
    xrotate[0] = cosl(thetarad) * x[0] - sinl(thetarad) * x[1];
    xrotate[1] = sinl(thetarad) * x[0] + cosl(thetarad) * x[1];
}
...
int main() {
    ...
    fp1 = fopen("../datafiles/kimkye_rpd_funct_combine_xy_2.dat", "w");
    ...
    long double *xnotrot = calloc(2, sizeof(long double));
    long double *xrot = calloc(2, sizeof(long double));

    for (unsigned int i = 0; i < nbins - 1; i++) {
        for (unsigned int j = 0; j < nbins - 1; j++){
            xnotrot[0] = xi[i];
            xnotrot[1] = yi[j];
            rotate_state(xnotrot, 45, xrot);
            fprintf(fp1, "%5.10Lf %5.10Lf %5.10Lf %5.10Lf %5.10Lf\n", xi[i], yi[j], xrot[0], xrot[1], rpdi[i][j]);
        }
    }
